        ...
        <el-form ref="loginForm" style="width: 30rem">
          <el-form-item>
            <el-input
              v-model="urls"
              type="textarea"
              placeholder="请输入链接 多个链接用回车分开 越多链接会花费越多时间"
          ...
          <el-form-item>
            <el-button type="primary" @click="addSome"> 提交 </el-button>
          </el-form-item>
        </el-form>
      </div>
    </el-main>
...
export default {
  ...
  methods: {
    addSome() {
      ...
      axios
        .post(this.url + "/add", qs.stringify({ urls: this.urls }), {
          headers: {
        ...
        .then((res) => {
          ...
          this.alertMessage(
            "成功数量 " +
              String(res.data.successNum) +
              " | 提交数量" +
              String(res.data.submitNum)
          );
          this.urls = "";
        })
        .catch((err) => {
